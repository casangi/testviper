...
- Each workflow row includes:
  ...
  - Relative run time
  - Failure rate pill (last N completed runs; N = `stats_sample` from baked data, default 20)
...
- If pre-baked data is available and not force-refreshed:
  - Render row status, times, failure rate pills, and average durations from embedded JSON immediately.
...
- For a `ci` category, dashboard shall call one endpoint per configured workflow file via the Worker proxy:
  - `GET /github/repos/{owner}/{repo}/actions/workflows/{file}/runs?per_page=20&status=completed`
- Panel shall show per-workflow: status dot, job label, branch, human-readable status, relative time, failure rate pill, and average duration.
- Failure rate and average duration are computed from the same 20-run sample used for the latest status. A minimum of 5 completed runs is required before these stats are emitted; below that threshold both values are `null` and rendered as `—`.
- Panel footer shall display two links on the same horizontal row:
  - "View in Insights ↗" (left) — links to the org-level GitHub Actions Insights page.
...
- `stats_sample`: number of completed runs fetched per workflow for statistics (default 20)
...
- `projects[projectId].workflows[file].failure_rate`: float (0–100) or `null` if fewer than 5 completed runs
- `projects[projectId].workflows[file].avg_duration`: integer seconds or `null` if no timing data available
...
### AC-10: CI statistics columns

- CI overview table and CI detail panel both render failure rate pills and average duration columns.
- Failure rate pill colour reflects severity: green (< 5 %), amber (5–20 %), red (> 20 %), grey dash when insufficient data (< 5 completed runs).
- Average duration is formatted as human-readable time (e.g., `4m 30s`, `1h 05m`); `—` when unavailable.
